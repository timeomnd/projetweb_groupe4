...
with open('csv/communes-france-2024-limite.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        if reg_code not in regions_deja_inseres:
            sqlfile.write(f"INSERT IGNORE INTO Region (reg_code, reg_nom) VALUES ('{reg_code}', '{reg_nom}');\n")
            regions_deja_inseres.add(reg_code)

        if dep_code not in departements_deja_inseres:
            sqlfile.write(
                f"INSERT IGNORE INTO Departement (dep_code, dep_nom, reg_code) VALUES ('{dep_code}', '{dep_nom}', '{reg_code}');\n"
            )
            departements_deja_inseres.add(dep_code)

        sqlfile.write(
            f"INSERT IGNORE INTO Commune (code_insee, nom_standard, code_postal, population, dep_code) "
