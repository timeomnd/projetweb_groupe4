...
with open("csv/data.csv", newline='', encoding="utf-8") as csvfile:
    ...
    for row in reader:
        ...
        cursor.execute("SELECT id_modele_panneau FROM Modele_panneau WHERE nom_modele = %s AND id_marque_panneau = %s", (modele_panneau, id_marque_panneau))
        res = cursor.fetchone()
        if res:
            id_modele_panneau = res[0]
        else:
            cursor.execute("INSERT INTO Modele_panneau (nom_modele, id_marque_panneau) VALUES (%s, %s)", (modele_panneau, id_marque_panneau))
        ...
        cursor.execute("SELECT id_modele_onduleur FROM Modele_onduleur WHERE nom_modele = %s AND id_marque_onduleur = %s", (modele_onduleur, id_marque_onduleur))
        res = cursor.fetchone()
        if res:
            id_modele_onduleur = res[0]
        else:
            cursor.execute("INSERT INTO Modele_onduleur (nom_modele, id_marque_onduleur) VALUES (%s, %s)", (modele_onduleur, id_marque_onduleur))
